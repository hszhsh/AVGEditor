...
export function PlotScene() {
    const [state, setState] = React.useState<{ plot: PlotModel | undefined, dialogueIndex: number }>();
    ...
    React.useEffect(() => {
        ...
        const startPlot = async (plotId) => {
            ...
            setState({ plot: model, dialogueIndex: 0 });
        };
        if (state) {
    ...
    if (state) {
        return <DialogueSceneView scene={state.plot.dialogues[state.dialogueIndex]} onEnd={() => {
            let newDialogue = state.dialogueIndex + 1;
            if (newDialogue === state.plot.dialogues.length) {
                startNextPlot(state.plot);
            } else {
                setState({ plot: state.plot, dialogueIndex: newDialogue });
            }
        }} />
    }
    else {
