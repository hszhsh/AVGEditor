...
function TextView(props: TextElementProps & { nativeRef: React.RefObject<View> }) {
    ...
    return <avgtext ref={props.nativeRef} {...props} text={text}>
        {props.children}
    </avgtext>;
}
...
class DisplayView extends AnimatedViewBase<DisplayViewProps, DisplayViewState> {
    ...
    stateFromProps(props: DisplayViewProps) {
        ...
        let ret = this.state ? { viewProps: { ...this.state.viewProps, ...props.viewProps } } : { viewProps: { ...props.viewProps } };
        if (props.sceneNode) { // 动画更新属性不包含sceneNode字段
    ...
    renderChildren = () => {
        return this.props.sceneNode.children &&
            this.props.sceneNode.children.map(v => <DisplayView key={v.key} id={v.key} sceneNode={v} viewProps={v.view} />)
    }
    ...
    render() {
        ...
        if (Tag === "avgtext") {
            return (
                <TextView nativeRef={this.viewRef} {...props}>
                    {this.renderChildren()}
                </TextView>
            )
        }
        if (Tag === "plotbutton") {
            return <avgbutton ref={this.viewRef as any} {...props} onClick={async () => {
            ...
            return <avgbutton ref={this.viewRef as any} {...props} onClick={() => {
                if (this.context.actionManager.inTransition) return;
                let event = (this.props.sceneNode.view as ButtonNodeType).event;
                if (event) {
                    GameEvents.emitter.emit(event);
...
export function DialogueSceneView(props: { scene: DialogueModel, onEnd: (plotButtonKey?: string) => void }) {
    for (let node of props.scene.sceneNodes) {
        alignView(node);
    }
    return <AnimatedRootView action={props.scene.action} onEnd={props.onEnd}>
        {props.scene.sceneNodes.map(v => <DisplayView key={v.key} id={v.key} sceneNode={v} viewProps={v.view} />)}
    </AnimatedRootView>
}