...
import { TweenEasingType } from "@/renderer/types/action-types";
import { richNodeTextCount, richNodesWithLengthLimit } from "@/renderer/common/richtext";
...
export class TextAnimator extends Animator<TextProps> {
    ...
    constructor(props: TextProps, onChange: (value: RichTextNode[]) => void) {
        ...
        this.textCount = richNodeTextCount(props.text);
    }
    ...
    update = (dt: number) => {
        ...
        if (textCount != this.currTextCount) {
            ...
            this.valueCallback(richNodesWithLengthLimit(this._props.text, textCount));
    ...
    start() {
        ...
        if (this.textCount == 0 || this._props.speed <= 0) {
            this.valueCallback(this._props.text as RichTextNode[]);
            return Promise.resolve();
        }
        GraphicsGL.scheduleUpdate(this.update);
    ...
    stop() {
        GraphicsGL.unscheduleUpdate(this.update);
    }
