...
import { FiberType, MouseEventType, Position, TouchEventType, KeyboardEventType } from "../Types";
import { CocosGL } from "./CocosGL";

type MouseEvent = import("../views/View").MouseEvent;
type TouchEvent = import("../views/View").TouchEvent;
type KeyboardEvent = import("../views/View").KeyboardEvent;
...
function getKeyboardEventType(type: string) {
    if (type === "keydown") return KeyboardEventType.KeyDown;
    else if (type === "keyup") return KeyboardEventType.KeyUp;
    throw "Invalid keyboard event type " + type;
}
...
export default class CCView implements View {
    ...
    protected keyboardEventListener?: (e: KeyboardEvent) => void;
    ...
    protected registerKeyboardEvents() {
        cc.systemEvent.on(cc.SystemEvent.EventType.KEY_DOWN, this.handleKeyboardEvent);
        cc.systemEvent.on(cc.SystemEvent.EventType.KEY_UP, this.handleKeyboardEvent);
    }

    protected unregisterKeyboardEvents() {
        cc.systemEvent.off(cc.SystemEvent.EventType.KEY_DOWN, this.handleKeyboardEvent);
        cc.systemEvent.off(cc.SystemEvent.EventType.KEY_UP, this.handleKeyboardEvent);
    }
    ...
    handleKeyboardEvent = (event: cc.Event.EventKeyboard) => {
        if (this.onKeyboardEvent) {
            let _event: KeyboardEvent = {
                type: getKeyboardEventType(event.type),
                keyCode: event.keyCode,
            }
            this.onKeyboardEvent(_event);
        }
    }
    ...
    set color(color: { r: number; g: number; b: number; a?: number | undefined; }) {
        this._node.color = new cc.Color(color.r * 255, color.g * 255, color.b * 255, color.a !== undefined ? color.a * 255 : 255);
    }
    get color() {
    ...
    set onKeyboardEvent(func: ((e: KeyboardEvent) => void) | undefined) {
        this.keyboardEventListener = func;
        if (func) {
    ...
    captureToTexture(): Promise<Texture> {
        return new Promise((resolve) => {
            const width = this._node.width * (<any>cc.view)._scaleX;
            const height = this._node.height * (<any>cc.view)._scaleY;
            ...
            camera.node.position = cc.v3(this._node.width / 2 - this._node.anchorX * this._node.width,
                this._node.height / 2 - this._node.anchorY * this._node.height, 800);
